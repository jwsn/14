...
PosBase* TowerPosEditorLayer::findExistMonsterPos(Point pos)//根据坐标找到已经存在的塔坐标
{
...
void TowerPosEditorLayer::createMonsterPos(Point pos)//给定坐标生成塔坐标
{
	MonsterPos* mPos = MonsterPos::create(pos, true);
	this->addChild(mPos, 10);
	m_monsterPosList.pushBack(mPos);
}
void TowerPosEditorLayer::deleteMonsterPos(PosBase* existPos)//给定塔坐标，删除塔坐标对象
{
	this->removeChild(existPos);
